...
On this page we present the details of the model. The model consists of three
...
$$\mu_{g.i} = \max{\left(\text{gear_mort}_i - \mu_{f.i}(w), 0\right)}
$$
...
$p_{C.ext}$ comes from animals that have died by natural causes other than predation ("external"):
  ...
  $$p_{C.ext} = \text{ext_prop}\,\sum_i\int\mu_{ext.i}(w)N_i(w)w\,dw.$$
