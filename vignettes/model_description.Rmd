...
components: 1) a collection of species modelled by [size spectra](#species-size-spectra),
2) [carrion] and 3) [detritus]. We discuss each of these in turn below.



# Parameter values
In addition to the description of the general model, this document also
includes sections that give numerical values for the model parameters
introduced in the previous section. These are the parameter values we chose
for describing the shelf ecosystem off the coast of Blanes
in the Northwestern Mediterranean. These sections are all headed "Parameter
values" and start with a "Skip" link that allows you to jump over them to
the next section of model description. We start with such a section that
describes the observed abundances of the species included in the model
as well as the carrion and detritus.

[Skip this section](#species-size-spectra)

We model `r nrow(params@species_params)` species, in addition
...
params@species_params %>%
    ...
    kable(col.names = c("biomass", "cutoff size"))
```
...
params@species_params %>%
    ...
    kable(col.names = c("numbers", "cutoff size"))
```

While the observed abundances of the species are not directly model parameters, they have been used to choose the reproduction parameters discussed later so
that the steady state abundances in the model agree with these observations.

In the steady state the total carrion biomass per m^2 is \( B_C = `r signif(carrion_biomass(params), digits = 4)`\) grams. This was chosen so that
the expected lifetime for the carrion biomass,
i.e., the inverse of the mass-specific carrion consumption rate, is equal
to `r signif(carrion_lifetime(params)*365, digits = 3)` day.

In the steady state the total detritus biomass per m^2 is \( B_D = `r signif(detritus_biomass(params), digits = 4)`\) grams. This was chosen so that 
the expected lifetime for the detritus biomass,
i.e., the inverse of the mass-specific detritus consumption rate, is 
`r signif(detritus_lifetime(params), digits = 4)` year.
...
grams per year, the prefactor $\gamma_i$ has to have a unit of 
$\rm{grams}^{-3/4}$ per year.
...
$h_i(w) = h_i\, w^n$
with $n=0.7$. The unit of the coefficients $h_i$ are $\rm{grams}^{1-n}$ per year.
...
The units of the coefficients $k_{s.i}$ are $\rm{grams}^{1-p}$ per year and
the units of the $k_i$ is grams per year.
...
relationship $w = a l^b$ where $w$ is measured in grams and $l$ is measured
in centimetres.
...
### Fishing mortality
The fishing mortality rate $\mu_{f.i}(w)$ is the product of the species- and size-dependent
selectivity of the gear, the species-specific catchability and the fishing effort.

We use two types of selectivity curves: knife-edge selectivity where the selectivity is zero until a certain knife-edge size where it jumps to 1 (fully
selected) and sigmoidal selectivity where the transition from 0 to 1 is more
gradual. The sigmoidal selectivity is described by the parameters `l50` which is
the lenght in centimetres where 50% of the individuals are selected and `l25`,
the length at wich 25% are selected. 

We choose a normalisation where the current fishing effort is taken to be equal to 1 so that the `catchability` gives the
fishing mortality rate at fully selected sizes. 

#### Parameter values
Skip to [Excess gear mortality]

The species for which we use sigmoidal selectivity are:
```{r}
params@gear_params %>%
...
```
We use knife-edge selectivity for the following species:
```{r}
params@gear_params %>%
    ...
    kable(row.names = FALSE,
          col.names = c("Species", "knife edge size [g]", "catchability [1/year]"))
```
The remaining species experience no fishing mortality:
`r paste(params@gear_params$species[params@gear_params$catchability == 0], collapse = ", ")`.
...
$$\mu_{g.i} = \max{\left({\tt gear\_mort}_i - \mu_{f.i}(w), 0\right)}
$$
...
The following table also gives the steady-state reproduction level which is 
defined as the ratio between the actual reproduction rate $R_i$ and the maximal
possible reproduction rate $R_{\max.i}$.
```{r}
...
The reproduction parameters `erepro` and `R_max` are not directly observable.
The values were instead chosen so as to give reasonable values for the
reproduction level and to produce steady-state abundances of the species that
...
The value of the mass-specific rate of decomposition is 
$d_C=`r params@other_params$carrion$decompose`$ per year. This was chosen so
that the production and consumption are equal for the chosen steady state 
abundances.
...
$p_{C.ext}$ comes from animals that have died by natural causes other than predation ("external"):
  ...
  $$p_{C.ext} = {\tt ext\_prop}\,\sum_i\int\mu_{ext.i}(w)N_i(w)w\,dw.$$
...
The detritus spectrum stretches from  \(w_0=`r signif(min(params@w_full), digits = 1)`\)
to \(w_{cutoff}=`r params@resource_params$w_pp_cutoff`\) grams.
The power law exponent is \(\lambda=`r params@resource_params$lambda`\).
...
$\theta_{jD}=`r signif(params@species_params$interaction_resource[[1]], 3)`$
for all predator species. Note that this does not mean that all species are
detritivores. For most species the predation kernel will be such that detritus will only be selected by the very small individuals.
...
#### Parameter values
The value of the external detritus production rate is $p_{d.ext}=`r
signif(params@other_params$detritus$external, digits = 4)`$ grams per year. This
was chosen so that the production and consumption are equal for the chosen
