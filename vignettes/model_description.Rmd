...
to $w_{cutoff}=`r params@resource_params$w_pp_cutoff`$ gramms.
The power law exponent is $\lambda=`r params@resource_params$lambda`$.
In the steady state the total detritus biomass is $B_D=`detritus_biomass(params)`$.
</details>
...
$p_{d.ext}=`r params@other_params$detritus$external`$ grams per year. 
...
$$\mu_{D}(w_p) = \sum_j  \theta_{jD}\int_{w_0}^{w_cutoff} \phi_j(w,w_p) (1-f_j(w))
  \gamma_j(w) N_j(w) \, dw.
$$
