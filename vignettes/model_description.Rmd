...
#| warning: false
# A few of the fastest-growing small invertebrates reach their asymptotic
# size in well under a year, which makes the ODE solver used by
# `getGrowthCurves()` fail when integrating over the default 20-year age
# range used for all species. We therefore only plot the species for which
# the growth curve can actually be computed.
plottable <- vapply(params@species_params$species, function(sp) {
    result <- NULL
    invisible(capture.output(
        result <- tryCatch(getGrowthCurves(params, species = sp),
                            error = function(e) e)
    ))
    !inherits(result, "error")
}, logical(1))
plotGrowthCurves(params, species = names(plottable)[plottable],
                 species_panel = TRUE)
```
