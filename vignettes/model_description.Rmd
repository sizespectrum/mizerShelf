...
In the steady state the total carrion biomass is \( B_C = `r signif(carrion_biomass(params), digits = 4)`\) grams. This was chosen so that
...
In the steady state the total detritus biomass is \( B_D = `r signif(detritus_biomass(params), digits = 4)`\) grams. This was chosen so that 
...
$d_C=`r params@other_params$carrion$decompose`$ per year. 
...
#### Parameter values
The value of the external detritus production rate is 
$p_{d.ext}=`r signif(params@other_params$detritus$external, digits = 4)`$ grams per year. 
