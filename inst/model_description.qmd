...
observed. @tbl-biomass gives the observed biomass per square meter
...
observed instead. These are shown in @tbl-numbers.
```{r}
#| label: tbl-numbers
...
For the commercial species we also have the yearly fishery yield, given in @tbl-yields in grams per square meter per year. For these species we calibrated the model abundances so that the estimated fishing mortalities lead to these yields.
```{r}
#| label: tbl-yields
...
The parameters for the predation kernels were estimated from stomach data or from the physical characteristics of the species. For the species that use a lognormal predation kernel, the parameters are given in @tbl-lognormal. The values for the detritivores were chosen so that they would have access to detritus throughout their life.
...
For the species that use a truncated power law predation kernel, the parameters are given in @tbl-power.
...
A proportion $\psi_i(w)$ of the energy available for growth and reproduction is used for reproduction. This proportion changes from zero below the weight $w_{m.i}$ of maturation to one at the asymptotic weight $w_{\infty.i}$, where all available energy is used for reproduction. The expression is 
$$ 
\psi_i(w) = \begin{cases}
...
The values for the model parameters were chosen so that the resulting growth curves would be close to von Bertalanffy growth curves. The paramters in @tbl-kvb were taken from the literature.
...
The mortality rate $\mu_i(w)$ of an individual of species $i$ and weight $w$ has four sources: predation mortality $\mu_{p.i}(w)$, external mortality $\mu_{ext.i}(w)$, fishing mortality $\mu_{f.i}(w)$ and excess gear mortality $\mu_{g.i}$, which combine as
$$
...
External mortality $\mu_{ext.i}(w)$ is independent of the abundances. By default, mizer assumes that the external mortality is a species-specific constant $z0_i$ independent of size. The value of $z0_i$ is either specified as a species parameter or it is assumed to depend allometrically on the asymptotic size: 
$$
...
We use the size-independent external mortalities give in @tbl-extmort.
...
Three important density-dependent mechanisms widely assumed in fisheries models are automatically captured in the mizer model, which lead to an emergent stock-recruitment relationship:
...
@tbl-reproduction gives the steady-state reproduction level which is defined as the ratio between the actual reproduction rate $R_i$ and the maximal possible reproduction rate $R_{\max.i}$.
...
```{r}
#| label: tbl-rho
#| tbl-cap: Parameters determining rates of carrion consumption.
params@species_params %>%
    select(rho_carrion) %>%
